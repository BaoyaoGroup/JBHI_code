...
def train_model(
    ...
    elif "isic2016" in str(dir_img):
        dataset = ISIC2016Dataset(dir_img, dir_mask, img_scale)
    elif "isic2017" in str(dir_img):
        dataset = ISIC2017Dataset(dir_img, dir_mask, img_scale)
    else:
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f"Epoch {epoch}/{epochs}", unit="img") as pbar:
            for batch in train_loader:
                ...
                images = torch.cat((zeros_tensor, images), dim=1)
...
def get_args():
    ...
    parser.add_argument(
        "--dataset", "-d", metavar="D", type=str, default="carvana", help="Dataset name"
    )
    parser.add_argument(
        "--batch-size",
        "-b",
    ...
    parser.add_argument(
        "--num_groups", "-g", type=int, default=0, help="Number of groups"
    )
    parser.add_argument("--model", "-m", type=str, default="qgd_net", help="Model name")
...
if __name__ == "__main__":
    ...
    dir_img = Path(f"./data/{DATASET}/imgs/")
    dir_mask = Path(f"./data/{DATASET}/masks/")
    dir_checkpoint = Path(
        f"./checkpoints/{args.model}/{DATASET}/num_groups_{args.num_groups}"
    )
    ...
    logging.info(
        ...
        f"\t{args.num_groups} groups\n"
        f'\t{"Bilinear" if model.bilinear else "Transposed conv"} upscaling'
    )
    experiment = wandb.init(
        project=model.name, resume="allow", anonymous="must", config=args
    )
